...
/*
 * ufshcd_crypto_cap_find - find a crypto capability index matching the given
 * blk-crypto mode and data unit size, by scanning the device-reported
 * crypto_cap_array. Used by vendor crypto backends (e.g. ufshcd-crypto-qti.c)
 * that program keys through their own SCM/ICE path rather than through
 * ufshcd_crypto_keyslot_program() below.
 */
